# Mapping of single-end reads to maize genome version NAM-5.0
...
for srr in $(cat $file)
  do
  ulimit -n 10000
	STAR --runThreadN 24 --readFilesIn ${srr}.fastq.gz \
...
for srr in $(cat $file)
  do
  ulimit -n 10000
	STAR --runThreadN 24 --readFilesIn ${srr}-trim.fastq.gz \
--genomeDir star-index --outSAMtype BAM SortedByCoordinate --outFileNamePrefix ${srr} --readFilesCommand zcat
bamqc ${srr}Aligned.sortedByCoord.out.bam --threads 24 -f Zea_mays.Zm-B73-REFERENCE-NAM-5.0.55.chr.gtf \
-o raw_bamqc/
done
